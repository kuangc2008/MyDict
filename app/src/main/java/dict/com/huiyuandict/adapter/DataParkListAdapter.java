...
public class DataParkListAdapter extends BaseAdapter {
    ...
    public DataParkListAdapter(Context mContext, List<Data> datas) {
        ...
        int index = 0;
        while (index < size) {
            String currentLetter = ((Data) datas.get(index)).getPart();
            if (!currentLetter.equalsIgnoreCase(index >= 1 ? ((Data) datas.get(index - 1)).getPart() : "")) {
    ...
    public View getView(int position, View view, ViewGroup parent) {
        ...
        if (position >= 0) {
            ...
            String currentLetter = ((Data) this.datas.get(position)).getPart();
            if (currentLetter.equalsIgnoreCase(position >= 1 ? ((Data) this.datas.get(position - 1)).getPart() : "")) {
                holder.letter.setVisibility(8);
            } else {
                holder.letter.setVisibility(0);
                holder.letter.setText(currentLetter);
            }
            holder.item.setOnClickListener(DataParkListAdapter$$Lambda$1.lambdaFactory$(this, position));
        }
        return view;
    }

    static /* synthetic */ void lambda$getView$0(DataParkListAdapter this_, int position, View v) {
